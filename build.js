...
function buildPlugin() {
    ...
    lines.push('');
    lines.push('// 在线模式配置');
    lines.push('let onlineConfig = {');
    lines.push('    enabled: false,');
    lines.push("    serverUrl: 'https://room.yufugemini.cloud',");
    lines.push('    currentRoomId: null,');
    lines.push('    currentRoomPort: null,');
    lines.push('};');
    lines.push('');
    lines.push('// HTTP 请求工具');
    lines.push('const https = require("https");');
    lines.push('const http = require("http");');
    lines.push('');
    lines.push('function fetchApi(method, path, body = null) {');
    lines.push('    return new Promise((resolve, reject) => {');
    lines.push('        const url = new URL(path, onlineConfig.serverUrl);');
    lines.push('        const isHttps = url.protocol === "https:";');
    lines.push('        const options = {');
    lines.push('            hostname: url.hostname,');
    lines.push('            port: url.port || (isHttps ? 443 : 80),');
    ...
    lines.push('                try {');
    lines.push('                    resolve({ status: res.statusCode, data: JSON.parse(data) });');
    lines.push('                } catch (e) {');
    lines.push('                    resolve({ status: res.statusCode, data: data });');
    lines.push('                }');
    lines.push('            });');
    lines.push('        });');
    lines.push('        ');
    ...
    lines.push('            onlineMode: onlineConfig.enabled,');
    lines.push('            onlineServerUrl: onlineConfig.serverUrl,');
    lines.push('        });');
    lines.push('    });');
    lines.push('    ');
    lines.push('    // ═══ 在线模式 API ═══');
    lines.push('    ');
    lines.push('    // 获取/设置在线模式配置');
    lines.push("    router.get('/api/multiplayer/online/config', (req, res) => {");
    lines.push('        res.json({');
    lines.push('            enabled: onlineConfig.enabled,');
    lines.push('            serverUrl: onlineConfig.serverUrl,');
    lines.push('            currentRoomId: onlineConfig.currentRoomId,');
    lines.push('        });');
    lines.push('    });');
    lines.push('    ');
    lines.push("    router.post('/api/multiplayer/online/config', (req, res) => {");
    lines.push('        const { enabled, serverUrl } = req.body;');
    lines.push('        if (typeof enabled === "boolean") onlineConfig.enabled = enabled;');
    lines.push('        if (serverUrl) onlineConfig.serverUrl = serverUrl;');
    lines.push('        res.json({ success: true, config: onlineConfig });');
    lines.push('    });');
    lines.push('    ');
    lines.push('    // 获取房间列表');
    lines.push("    router.get('/api/multiplayer/online/rooms', async (req, res) => {");
    lines.push('        try {');
    lines.push('            const result = await fetchApi("GET", "/rooms");');
    lines.push('            res.status(result.status).json(result.data);');
    lines.push('        } catch (error) {');
    ...
    lines.push('    ');
    lines.push('    // 创建房间');
    lines.push("    router.post('/api/multiplayer/online/rooms', async (req, res) => {");
    lines.push('        try {');
    lines.push('            const { name, password, maxUsers, creatorId, creatorName } = req.body;');
    lines.push('            const result = await fetchApi("POST", "/rooms", {');
    lines.push('                name, password, maxUsers, creatorId, creatorName');
    lines.push('            });');
    lines.push('            ');
    lines.push('            if (result.status === 201 && result.data.id) {');
    lines.push('                onlineConfig.currentRoomId = result.data.id;');
    lines.push('                ');
    lines.push('                // 生成 WebSocket 连接 URL');
    lines.push('                const baseUrl = onlineConfig.serverUrl.replace("https://", "wss://").replace("http://", "ws://");');
    lines.push('                result.data.wsUrl = `${baseUrl}/ws/room/${result.data.id}`;');
    lines.push('            }');
    lines.push('            ');
    lines.push('            res.status(result.status).json(result.data);');
    lines.push('        } catch (error) {');
    ...
    lines.push('    ');
    lines.push('    // 加入房间');
    lines.push("    router.post('/api/multiplayer/online/join', async (req, res) => {");
    lines.push('        try {');
    lines.push('            const { roomId, password } = req.body;');
    lines.push('            const result = await fetchApi("POST", `/rooms/${roomId}/join`, { password });');
    lines.push('            ');
    lines.push('            if (result.status === 200) {');
    lines.push('                onlineConfig.currentRoomId = roomId;');
    lines.push('                ');
    lines.push('                // 生成 WebSocket 连接 URL: wss://domain/ws/room/{roomId}');
    lines.push('                const baseUrl = onlineConfig.serverUrl.replace("https://", "wss://").replace("http://", "ws://");');
    lines.push('                result.data.wsUrl = `${baseUrl}/ws/room/${roomId}`;');
    lines.push('            }');
    lines.push('            ');
    lines.push('            res.status(result.status).json(result.data);');
    lines.push('        } catch (error) {');
    ...
    lines.push('    ');
    lines.push('    // 退出房间');
    lines.push("    router.post('/api/multiplayer/online/leave', async (req, res) => {");
    lines.push('        onlineConfig.currentRoomId = null;');
    lines.push('        onlineConfig.currentRoomPort = null;');
    lines.push('        res.json({ success: true });');
    lines.push('    });');
    lines.push('    ');
    lines.push('    // 删除房间');
    lines.push("    router.delete('/api/multiplayer/online/rooms/:roomId', async (req, res) => {");
    lines.push('        try {');
    lines.push('            const { roomId } = req.params;');
    lines.push('            const result = await fetchApi("DELETE", `/rooms/${roomId}`);');
    lines.push('            res.status(result.status).json(result.data);');
    lines.push('        } catch (error) {');
    lines.push('            res.status(500).json({ error: error.message });');
    lines.push('        }');
